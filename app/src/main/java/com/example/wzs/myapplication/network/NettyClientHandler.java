...
public class NettyClientHandler extends ChannelInboundHandlerAdapter {
    ...
    @Override
    public void channelActive(ChannelHandlerContext ctx) throws Exception { //channelActive()方法将会在连接被建立并且准备进行通信时被调用。
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        buf.readBytes(req);
        String body = new String(req, "UTF-8"); //verify(String body)方法对服务器返回的数据进行校验，并取出数据部分。
        //具体校验的方法需要与后台同事进行协议。
        Log.d(TAG, "verify : " + body);
        if (null != body) parseJson(body);
    }
    ...
    public void channelReadComplete(ChannelHandlerContext ctx)
        ...
        LogUtil.e(TAG, "channelReadComplete");
    }
    ...
    private void parseJson(String json) {
        try {
            ...
            LogUtil.e(TAG, "parseJson exception: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void callListeners(final int msgType, final String json) {
        for (final INettyClient.OnDataReceiveListener listener : listeners)
            if (listener != null) new Handler(Looper.getMainLooper()).post(new Runnable() {
                @Override
